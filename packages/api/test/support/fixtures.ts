...
export async function createExpense(db: Database, options: ExpenseOptions) {
  ...
  const status = options.status ?? (isSettled ? "settled" : "active");
  let cancelledAt: Date | null = null;
  let cancelledByUserId: string | null = null;

  if (status === "cancelled") {
    if (!options.cancelledBy) {
      throw new Error("cancelledBy is required for a cancelled expense");
    }

    cancelledAt = options.cancelledAt ?? now;
    cancelledByUserId = options.cancelledBy.userId;
  }

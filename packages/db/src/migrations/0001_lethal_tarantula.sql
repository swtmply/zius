CREATE TABLE `expense_item` (
	...
	FOREIGN KEY (`assigned_participant_id`) REFERENCES `participant`(`id`) ON UPDATE no action ON DELETE no action,
	CONSTRAINT "expense_item_position_check" CHECK("expense_item"."position" >= 0),
	CONSTRAINT "expense_item_quantity_check" CHECK("expense_item"."quantity" > 0),
	CONSTRAINT "expense_item_price_minor_check" CHECK("expense_item"."price_minor" >= 0)
);
--> statement-breakpoint
CREATE UNIQUE INDEX `expense_item_expense_position_uidx` ON `expense_item` (`expense_id`,`position`);--> statement-breakpoint
CREATE INDEX `expense_item_assigned_participant_id_idx` ON `expense_item` (`assigned_participant_id`);--> statement-breakpoint
...
CREATE TABLE `__new_expense` (
	...
	CONSTRAINT "expense_split_method_check" CHECK("__new_expense"."split_method" in ('equal', 'fixed', 'percentage', 'items')),
